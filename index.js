...
async function startPrompting() {
  ...
  let autoAnalyze = (
    await prompt(
      "Do you want to use ocr? (y/n) (n --> no image, type grid / no option for visualization): "
    )
  ).toLowerCase();
  while (autoAnalyze !== "n" && autoAnalyze !== "y") {
    autoAnalyze = (
      await prompt(
        "Do you want the puzzle to be analyzed automatically? (y/n): "
      )
    ).toLowerCase();
  }

  if (autoAnalyze.toLowerCase() === "y") {
    let imagePath = await prompt(
      "Name the relative path (e.g relative to index.js) to the grid puzzle image: "
    );

    let seperateLettersFromGridObj = await seperateLettersFromGrid(
      imagePath,
      "./letterSeperator",
      "./communicator",
      1 / 15,
      ...
      "none",
      null
    ); //,3300); // second (number param) 1/6 before // instead 35 was 50 , ....  4*8 maybe into percent of image ... (width and height)
    ...
    let markWords = (
      await prompt(
        "Do you want to mark specific words in the grid now? (y/n): "
      )
    ).toLowerCase();
    ...
    let searchWords = (
      await prompt(
        "Do you want to search for words from the dictionaries in the grid now? (y/n): "
      )
    ).toLowerCase();
    ...
    if (searchWords !== "n") {
      let markWordsFinal = (
        await prompt("Do you want to mark words in the grid now? (y/n): ")
      ).toLowerCase();
    ...
    async function markering() {
      let words = (
        await prompt(
          "Name the words you want to mark in the grid seperated by a comma (,): "
        )
      ).toLowerCase();
      while (words !== "n") {
    ...
    let userStr = (
      await prompt(
        "Type your grid here (like abcd,efgh -> , for a new row; letters must be directly next to each other (in a row)): "
      )
    )
      .toLowerCase()
      .replace(/\s/g, "");
    ...
    let quickCheck = (
      await prompt(
        "Do you just want to check wheter or not a word is in the grid? (y/n): "
      )
    ).toLowerCase();
    ...
    if (quickCheck === "y") {
      ...
      let allPossibilities = getAllPossibilities(grid, true)
        .map((obj) => obj.word)
    ...
    let dictionaryCheck = (
      await prompt("Auto analyzing for possible words? (y/n): ")
    ).toLowerCase();
...
async function getWordsFromGrid(strArr, rows, diagonals, minWordLength) {
  ...
  function loadGerman() {
    if (germanWords.length > 0) {
      ...
      let text = fs.readFileSync(
        "./languageResources/wordlists/german/german.txt",
        "utf-8"
      );
  ...
  function loadEnglish() {
    if (englishWords.length > 0) {
      ...
      /*return fetch(url)
                ...
                .then(json => {*/
      let text = fs.readFileSync(
        "./languageResources/wordlists/english/english.json",
        "utf-8"
      );
  ...
  let allPossibilities = getAllPossibilities(grid, diagonals).map(
    (obj) => obj.word
  );
  ...
  foundWords = foundWords.sort(function (a, b) {
    return b.length - a.length;
  });
  // sort found words by commonality
  let sort = sortArrByCommonality(foundWords);
  console.log(
    "final array sort by commonality with the help of a shorter dictionary (some common words might still be missed in ranking by commonality): "
  );
  ...
  function sortArrByCommonality(arr) {
    ...
    function isWordInFiles(arrPaths, word) {
      ...
      for (let i = 0; i < arrPaths.length; ++i) {
        ...
        if (keys.includes(word)) {
          ...
          // deeper search: e.g. when searching for car: is there a key combination with xx car or -car] or [car-, but simply not car as dictionary word itself (standing alone)
          for (let i = 0; i < keys.length; ++i) {
            let key = keys[i];
            if (key.includes(word)) {
              if (
                key.split(" ").includes(word) ||
                key.split("-").includes(word)
              ) {
                return true;
              }
    ...
    // Example usage
    const filePaths = [
      "./languageResources/dictionaries/EnglishToGerman.json",
      "./languageResources/dictionaries/GermanToEnglish.json",
    ];
    let common = [];
    let uncommon = [];
...
function getAllPossibilities(grid, diagonals) {
  // very shitty, especially how cells are handled (!!)
  ...
  function forwardWords() {
    ...
    for (let i = 0; i < grid.length; i++) {
      ...
      for (let j = 0; j < grid[i].length; j++) {
        // j is the beginning cell (column wise)
...
async function markWordsInGrid(strArr, rows, keyWords, xCrops, yCrops) {
  ...
  for (let i = 0; i < keyWords.length; ++i) {
    let indexArr = possibilities
      .filter((obj) => obj.word.toLowerCase() === keyWords[i].toLowerCase())
      .map((obj) => possibilities.indexOf(obj));

    for (let j = 0; j < indexArr.length; ++j) {
      // if one letter, found multiples times as the same cell
      index = indexArr[j];
      if (index !== -1) {
        let obj = {};
        ...
        function generateColor() {
          ...
          do {
            ...
            Math.abs(r - b) < 50 ||
            Math.abs(g - b) < 50
          ); // for contrast to white background
  ...
  if (cellsCollection.length > 0) {
    // ()
    await markCellsWithXandYCrops(
      "./communicator/visualizedCropping.png",
      "./communicator/wordVisualizations.png",
      xCrops,
      yCrops,
      cellsCollection
    ); // maybe as param safe location !!!
  }
  console.groupEnd();
}
async function markCellsWithXandYCrops(
  inputPath,
  outputPath,
  xCrops,
  yCrops,
  ...
  await new Promise((resolve) => {
    ...
    for (let i = 0; i < xCrops.ends.length; ++i) {
      resXCrops.push({
        left:
          i !== 0 ? Math.ceil(xCrops.ends[i - 1]) : Math.ceil(xCrops.leftBegin),
        width:
          i !== 0
            ? i !== xCrops.ends.length - 1
              ? Math.ceil(xCrops.ends[i] - xCrops.ends[i - 1])
              : Math.ceil(xCrops.rightEnd - xCrops.ends[i - 1])
            : Math.ceil(xCrops.ends[0] - xCrops.leftBegin),
      });
    }
    let resYCrops = [];
    for (let i = 0; i < yCrops.ends.length; ++i) {
      resYCrops.push({
        top:
          i !== 0 ? Math.ceil(yCrops.ends[i - 1]) : Math.ceil(yCrops.topBegin),
        height:
          i !== 0
            ? i !== yCrops.ends.length - 1
              ? Math.ceil(yCrops.ends[i] - yCrops.ends[i - 1])
              : Math.ceil(yCrops.bottomEnd - yCrops.ends[i - 1])
            : Math.ceil(yCrops.ends[0] - yCrops.topBegin),
      });
    }
...
async function seperateLettersFromGrid(
  grid,
  outputDir,
  communicatorDir,
  XFilledRequired,
    ...
    sharp(imgPath)
      ...
      .toBuffer(async (err, buffer, info) => {
        ...
        if (backgroundMoreTo === "black") {
          console.log(
            "background is closer to black, will need to invert image"
          );
          await new Promise((resolve, reject) => {
            sharp(imgPath)
              .negate({ alpha: false })
        ...
        async function deletedTempDirectory() {
          ...
          async function drawLines(
            ...
            try {
              ...
              console.log(
                "Vertical lines drawn successfully! - cropping visualized and saved in communicator folder"
              );
            } catch (err) {
              console.error(err);
            }
            ...
            await new Promise((resolve, reject) => {
              sharp(imgPath)
                .extract({
                  left:
                    i !== 0
                      ? Math.ceil(xCrops.ends[i - 1])
                      : Math.ceil(xCrops.leftBegin),
                  top: 0,
                  width:
                    i !== 0
                      ? i !== xCrops.ends.length - 1
                  ...
                  async (err, info) => {
                    ...
                    if (err) {
                      ...
                      if (i === xCrops.ends.length - 1) {
                        console.log(
                          "finished vertical cropping, now cropping all letters off of already cropped vertical parts"
                        );
          ...
          async function finish() {
            ...
            if (seperatingLinesFound) {
              console.log(
                "because of possible seperation lines, finals cropping will be zoomed in a bit"
              );
            }
            ...
            let finalX = [];
            for (let i = 0; i < xCrops.ends.length; ++i) {
              // finalX = xCrops.ends didn't work properly on push !!
            ...
            await drawLines(
              communicatorDir,
              finalX.map(Math.ceil),
              finalY.map(Math.ceil),
              filePath,
              [50, 205, 50]
            );
          }
          ...
          .toBuffer(async (err, buffer, info) => {
            ...
            for (let i = 0; i < yCrops.ends.length; ++i) {
              let toFile =
                outputDir +
                "/tempFinals/" +
                (i * allPathsLength + (myPartNum + 1)) +
                ".png"; // calculate number in query ...
              await new Promise((resolve, reject) => {
                if (moreLimitedImage) {
                  sharp(myPath)
                    .metadata()
                    .then((metadata) => {
                      ...
                      sharp(myPath)
                        .extract({
                          ...
                          top:
                            (i !== 0
                              ? Math.ceil(yCrops.ends[i - 1])
                              : Math.ceil(yCrops.topBegin)) + move,
                          width: info.width - move * 2,
                          height:
                            (i !== 0
                              ? i !== yCrops.ends.length - 1
                                ? Math.ceil(yCrops.ends[i] - yCrops.ends[i - 1])
                                : Math.ceil(
                                    yCrops.bottomEnd - yCrops.ends[i - 1]
                                  )
                              : Math.ceil(yCrops.ends[0] - yCrops.topBegin)) -
                            move * 2,
                        })
  ...
  function getYCrops(colorBackground, pixels) {
    ...
    console.log(
      "picked the following as supposed streak-thickness for the letters with exact occurance - number: "
    );
    ...
    function getResultingStreakYEnds() {
      ...
      for (let i = 0; i < streaks.length; i++) {
        if (
          Math.abs(streaks[i][0] - mostOften[0]) <=
          pixels.length * streakMaxDiff
        ) {
      ...
      console.log(
        "found " +
          finalStreaks.length +
          " streaks after similarity streak check"
      );
      for (let i = 0; i < finalStreaks.length; ++i) {
        if (i !== finalStreaks.length - 1) {
          ...
          if (i === 0) {
            ...
            // (--> if finalStreaks.length == 2 --> not called --> therefore and for safety !-call)
  ...
  function getBackgroundColorFromTwoColors(pixels) {
    ...
    function getMostFrequentColor(color1, color2, colors) {
      // ()
      const color1Count = colors.filter(
        (c) => getDistance(c, color1) < 50
      ).length; // Anzahl der Farben im Array, die nahe an color1 liegen
      const color2Count = colors.filter(
        (c) => getDistance(c, color2) < 50
      ).length; // Anzahl der Farben im Array, die nahe an color2 liegen
        ...
        .toBuffer((err, buffer, info) => {
          ...
          if (filter === "ridge3x3") {
            pixels = convolutionMatrix(
              ...
              false,
              pixels
            ); // use convolution matrix on it (ridge detecten 3x3), color addent 100, ...
  ...
  function convolutionMatrix(
    ...
    for (let t = 0; t < times; t++) {
      if (
        Math.sqrt(pattern.length) % 1 !== 0 ||
        Math.sqrt(pattern.length) % 2 !== 1
      ) {
        System.out.println(
          "Error: square of 'length of 'pattern'' is a decimal OR square of 'length of 'pattern'' is even"
        );
        return null;
      }
      ...
      for (
        let x = patternSqrt_xy0;
        x + (patternSqrt - 1) / 2 < width;
        x += offset
      ) {
        // Prozedere für jeden Pixel für den der Filter anwenndbar ist
        patternSqrt_y0_p = 0;
        for (
          let y = patternSqrt_xy0;
          y + (patternSqrt - 1) / 2 < height;
          y += offset
        ) {
          let sumR = 0;
          let sumG = 0;
          ...
          if (median) {
            ...
            for (let x1 = 0; x1 < patternSqrt; x1++) {
              // einzelne Pixel innerhalb der Reichweite des Filters anschauen, um damit den Farbwert des neuen zu bestimmen
              for (let y1 = 0; y1 < patternSqrt; y1++) {
                pixelFilterR[counter] =
                  pixel[patternSqrt_x0_p + x1][patternSqrt_y0_p + y1].r *
                  pattern[counter]; // normalerweise immer 1
                pixelFilterG[counter] =
                  pixel[patternSqrt_x0_p + x1][patternSqrt_y0_p + y1].g *
                  pattern[counter];
                pixelFilterB[counter] =
                  pixel[patternSqrt_x0_p + x1][patternSqrt_y0_p + y1].b *
                  pattern[counter];
                counter += 1;
              }
            }
          ...
          if (pixelating) {
            for (let x1 = 0; x1 < patternSqrt; x1++) {
              // jeden Pixel im Filter auf den Ergebniswert setzen, wenn pixelating == true
              for (let y1 = 0; y1 < patternSqrt; y1++) {
      ...
      if (!pixelating) {
        for (let m = 0; m < 2; m++) {
          // alle Ränder bestimmen mit nahem bestimmtem Farbwert
          ...
          for (let n = 0; n < 2; n++) {
            // System von 1 und 0 benutzt, ähnlich zu if Abfragen: wenn m = 0 --> 0+x<- ; m = 1 --> 1*(y-x)+x --> y<-
            for (let x = 0; x < m * (width - fLP) + fLP; x++) {
              for (let y = 0; y < m * (fTP - height) + height; y++) {
                if (y < fTP && m === 0) {
                  // Unterscheidungen: pixel über bzw. unter dem höchsten bestimmten filterbasierten Pixel müssen durch den naheliegensten bestimmten Pixel bestimmt werden
                  pixelNew[n * (fRP + 1) + x][y] = {
                    r: pixelNew[n * (fRP - fLP) + fLP][fTP].r,
                    g: pixelNew[n * (fRP - fLP) + fLP][fTP].g,
                    b: pixelNew[n * (fRP - fLP) + fLP][fTP].b,
                  };
                } else if (m * (x - y) + y <= m * (fRP - fBP) + fBP) {
                  if (m === 0) {
                    pixelNew[n * (fRP + 1) + x][y] = {
                      r: pixelNew[n * (fRP - fLP) + fLP][y].r,
                      g: pixelNew[n * (fRP - fLP) + fLP][y].g,
                      b: pixelNew[n * (fRP - fLP) + fLP][y].b,
                    };
                  } else {
                    pixelNew[x][n * (fBP + 1) + y] = {
                      r: pixelNew[x][n * (fBP - fTP) + fTP].r,
                      g: pixelNew[x][n * (fBP - fTP) + fTP].g,
                      b: pixelNew[x][n * (fBP - fTP) + fTP].b,
                    };
                  }
                } else if (m === 0) {
                  pixelNew[n * (fRP + 1) + x][y] = {
                    r: pixelNew[n * (fRP - fLP) + fLP][fBP].r,
                    g: pixelNew[n * (fRP - fLP) + fLP][fBP].g,
                    b: pixelNew[n * (fRP - fLP) + fLP][fBP].b,
                  };
                }
              }
            }
        ...
        for (let n = 0; n < 2; ++n) {
          if (fBP + n * (-fBP + fRP) !== height + n * (-height + width)) {
            for (
              let x = 0;
              x < width / offset + n * (-width / offset + height / offset);
              ++x
            ) {
              // x nicht immer für x bedeutend --> 0, 1 System für beide Ränder angewandt -- wahrscheinlich unübersichtlicher und ineffizienter als 2-Versionen davon zu schreiben, jedoch kürzer im Code
              let sumR = 0;
              let sumG = 0;
              let sumB = 0;
              for (
                let x1 = x * offset + n * (-(x * offset) + fRP);
                x1 < x * offset + offset + n * (-(x * offset + offset) + width);
                ++x1
              ) {
                for (
                  let y1 = fBP + n * (-fBP + x * offset);
                  y1 < height + n * (-height + x * offset + offset);
                  ++y1
                ) {
                  sumR += pixel[x1][y1].r;
                  sumG += pixel[x1][y1].g;
...
async function getLettersFromImages(dir, test) {
    ...
    await new Promise((resolve) => {
      (async () => {
        ...
        fs.readFile(path.join(dir, files[i]), function (err, src) {
          ...
          // now tessaract
          worker
            .recognize(path.join(dir, files[i]))
            .then(({ data: { text } }) => {
  ...
  function sortOutArrays(arr1, arr2) {
    // tesseract , ocrad
    let res = [];
